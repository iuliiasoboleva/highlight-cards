...
const STATUS_CONFIG = {
    ...
    cashback: [
        ...
        { label: 'Текущий процент кешбэка', valueKey: 'cashbackPercent', suffix: '%' },
    ...
    reward: [
        ...
        { label: 'Текущий уровень', valueKey: 'currentLevel', defaultValue: 'Нет данных' },
        { label: 'До следующей награды', valueKey: 'untilNextReward' },
        { label: 'Срок действия', valueKey: 'expirationDate' }
    ],
    stamp: [
        { label: 'До получения награды', valueKey: 'restStamps', suffix: ' штампов' },
        ...
        return inlineFields.map(({ label, valueKey, suffix = '', defaultValue = '' }) => {
            ...
            return value ? (
                <span
                    ...
                    title={label}
                >
                    <span className="inline-label">{shortLabel}:</span> {value}{suffix}
                </span>
            ) : null;
        });
    };
    ...
    return (
        <div className="card-info">
            <div className="card-info-header">
                ...
                <div className="card-inline-values">
                    {renderInlineValues()}
                </div>
            </div>
                    ...
                    return (
                        ...
                        value && (
                            <div key={valueKey} className="card-info-row">
                                <p className="card-info-row-label" title={label}>{shortLabel}:</p>
                                <p>{value}{suffix}</p>
                            </div>
                        )
                    );
